...
async function onGithubLoginClick() {
    try {
        ...
        if (data.url) {
            ...
            displayError('GitHub login is not configured');
        }
    } catch (err) {
        displayError('Failed to initiate GitHub login');
    }
}

// Check if redirected back from GitHub with error
const urlParams = new URLSearchParams(window.location.search);
const ghError = urlParams.get('error');
if (ghError === 'github_not_allowed') {
